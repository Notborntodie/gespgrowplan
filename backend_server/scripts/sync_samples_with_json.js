...
async function syncSamples() {
  ...
  try {
    ...
    console.log(`从 JSON 文件读取到 ${samples.length} 个测试样例\n`);
    
    await connection.beginTransaction();
    
    // 步骤1: 删除题目122的所有旧测试样例
    console.log('步骤1: 删除题目122的所有旧测试样例...');
    const [deleteResult] = await connection.execute(
      'DELETE FROM oj_samples WHERE problem_id = 122'
    );
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      // 只处理题目122的测试样例
      if (problem_id === 122) {
        console.log(`[${i + 1}/${samples.length}] 插入测试样例 (problem_id: ${problem_id}, sort_order: ${sort_order})`);
