...
async function queryCzyProgress() {
  const connection = await pool.getConnection();
  
  try {
    console.log('='.repeat(80));
    console.log('查询用户 czy 在"测试"计划的完成情况');
    console.log('='.repeat(80));
    console.log();
    ...
    for (const plan of plans) {
      ...
      if (planProgress.length > 0) {
        ...
        // 确保 is_completed 是数字或布尔值
        const isCompleted = Number(progress.is_completed) === 1 || progress.is_completed === true;
        console.log('📊 计划完成进度:');
        console.log(`   - 是否完成: ${isCompleted ? '✅ 是' : '❌ 否'}`);
        console.log(`   - 已完成任务: ${progress.completed_tasks || 0} / ${progress.total_tasks || 0}`);
      ...
      for (const task of tasks) {
        ...
        if (taskProgress.length > 0) {
          ...
          // 确保 is_completed 是数字或布尔值
          const isCompleted = Number(tp.is_completed) === 1 || tp.is_completed === true;
          console.log('   ✅ 任务完成状态:');
          console.log(`      - 是否完成: ${isCompleted ? '✅ 是' : '❌ 否'}`);
        ...
        const shouldBeCompleted = (totalExams === 0 || completedExams === totalExams) && 
                                   (totalOjs === 0 || completedOjs === totalOjs);
        const isTaskCompleted = taskProgress.length > 0 && 
                                (Number(taskProgress[0].is_completed) === 1 || taskProgress[0].is_completed === true);
        
        if (shouldBeCompleted && !isTaskCompleted) {
          console.log(`   ⚠️  问题: 所有题目都已完成，但任务未标记为完成！`);
          console.log(`      客观题: ${completedExams}/${totalExams}, OJ题: ${completedOjs}/${totalOjs}`);
        } else if (isTaskCompleted && !shouldBeCompleted) {
          console.log(`   ⚠️  问题: 任务已标记为完成，但题目未全部完成！`);
          console.log(`      客观题: ${completedExams}/${totalExams}, OJ题: ${completedOjs}/${totalOjs}`);
        } else if (shouldBeCompleted && isTaskCompleted) {
...
// 执行查询
queryCzyProgress().catch(console.error);
