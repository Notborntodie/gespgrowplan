<template>
    ...
    <div v-else class="exam-layout">
        ...
        <div class="question-main">
          <div class="question-card">
            ...
            <div class="question-content-unified">
              ...
              >
                <!-- 代码编辑器区域 -->
                <div class="code-editor-wrapper" ref="editorContainer" @click="focusEditor"></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- 语言解析弹窗 -->
      <div v-if="showAnalysisModal" class="result-modal-overlay" @click="closeAnalysisModal">
        <div class="result-modal" @click.stop>
          <div class="result-modal-header">
...
const renderMarkdown = (text: string): string => {
  ...
  try {
    ...
    // Step 2: 处理长字符串换行（但跳过已保护的数学公式占位符）
    ...
    // Step 3: 基础 Markdown 处理（但跳过已保护的数学公式占位符）
    let result = processed
      // 处理换行（但保留数学公式占位符中的换行标记）
      .replace(/\n/g, '<br>')
      // 处理代码块（需要在粗体/斜体之前处理，避免冲突）
      ...
      // 处理粗体（避免匹配数学公式占位符内的内容）
      .replace(/\*\*((?:(?!__MATH_)[\s\S])*?)\*\*/g, '<strong>$1</strong>')
      // 处理斜体（避免匹配数学公式占位符内的内容）
      // 使用兼容旧浏览器的方式，先检查前面不是*号
      .replace(/([^*]|^)\*((?:(?!__MATH_)[^*])+?)\*(?!\*)/g, (match, before, content, offset, string) => {
        // 检查前面不是*号（避免与粗体冲突）
      ...
      // 处理行内代码（避免匹配数学公式占位符）
      .replace(/`((?:(?!__MATH_)[^`])+?)`/g, '<code>$1</code>')

    // Step 4: 还原数学公式（只替换占位符，避免重复替换）
...
const confirmAndSubmit = async () => {
  ...
  const openAnalysis = () => {
    ...
    const samples = currentProblem.value.samples || []
    const samplePreview = samples.length > 0 ? `示例输入：\n\n\`\`\`\n${samples[0].input}\n\`\`\`\n\n示例输出：\n\n\`\`\`\n${samples[0].output}\n\`\`\`` : '（暂无示例）'
    analysisContent.value = [
      `### 解题思路（C++）`,
      `- 针对题目「${title}」（GESP ${level}级），建议先梳理输入输出格式，明确边界情况。`,
      `- 以标准输入输出为交互，优先考虑线性扫描或哈希表等 O(n) 解法；若涉及排序/区间，则考虑排序或双指针。`,
      `- 若样例不足以覆盖边界，请自制边界用例验证：空输入、极值、重复值、顺序反转等。`,
      `\n### 伪代码`,
      '```text\n读取输入\n处理数据与边界\n输出结果\n```',
      `\n### 复杂度分析`,
      `- 时间复杂度：依据核心操作，一般为 O(n) 或 O(n log n)。`,
      `- 空间复杂度：依据使用的数据结构，一般为 O(1) ~ O(n)。`,
      `\n### 样例参考`,
      samplePreview,
    ].join('\n\n')
